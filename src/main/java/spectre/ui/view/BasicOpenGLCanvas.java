...
import spectre.ui.view.screen.BaseScreen;
...
//        GL4 gl = glAutoDrawable.getGL().getGL4();
    ...
    public void drawScreen(GL2 gl) {
        ...
        if (printList != null) {
            for (PrintListNode pnl : printList.getPrintList()) {
                ...
                drawTriangleMesh(pnl.triangleMesh, gl);
                gl.glTranslatef(scale * x, scale * y, 0);
                gl.glPopName();
            }
    ...
    public void drawScreenBorder(GL2 gl, GLAutoDrawable glAutoDrawable) {
        ...
        for (int iter = 0; iter < 2; iter++) {
            drawTriangleMesh(cornerScreenGraphic, gl);

            for (int x = 0; x < 26; x++) {
                gl.glTranslatef(1.0f, 0.0f, 0.0f);
                drawTriangleMesh(midScreenGraphic, gl);
            }
    ...
    public void drawTriangleMesh(TriangleMesh tm, GL2 gl) {
        gl.glBegin(GL.GL_TRIANGLE_FAN);
        for (int i = 0; i < tm.vertices.length; i++) {
            if (tm.hasColorsA && tm.colors.length >= i)
                gl.glColor3f(tm.colors[i].getRed(), tm.colors[i].getGreen(), tm.colors[i].getBlue());

            if (tm.hasTexturesA && tm.texturePoints.length >= i)
                gl.glTexCoord2d(tm.texturePoints[i].x, tm.texturePoints[i].y);

            gl.glVertex3f(tm.vertices[i].x, tm.vertices[i].y, tm.vertices[i].z);
        }
        gl.glEnd();
    }
